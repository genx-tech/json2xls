var nodeExcel = require("@genx/excel-export");
...
transform.prepareJson = function (json, config) {
    ...
    if (!(fields instanceof Array)) {
        columnsMeta = Object.keys(fields).map(function (key) {
    ...
    res.cols = fields.map(function (key, i) {
        let columnType, columnCaption;
        
        if (columnsMeta.length > 0) {
            columnType = columnsMeta[i].type;
            columnCaption = columnsMeta[i].caption;
        } 
        ...
        return {
            ...
            type: getType(jsonArr[0][key], columnType),
            beforeCellWrite: function (row, cellData, eOpt) {
                eOpt.cellType = getType(cellData, columnType);
                return cellData;
            },
        };
    });
